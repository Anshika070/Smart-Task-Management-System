...
export default function App() {
  ...
  useEffect(() => { document.body.classList.toggle('dark', dark); localStorage.setItem('taskflow-theme', dark ? 'dark' : 'light'); }, [dark]);
  ...
  if (!user) return <AuthScreen isDark={dark} onToggleTheme={() => setDark(value => !value)} />;
  return <AppLayout activePage={page} setActivePage={setPage} onAdd={() => setEditing({})} user={user} onLogout={logout} isDark={dark} onToggleTheme={() => setDark(value => !value)}>{page === 'Categories' ? <CategoryOverview tasks={tasks} onOpenCategory={openCategory}/> : tasksView}{editing && <TaskFormModal task={editing.id ? editing : null} onClose={() => setEditing(null)} onSave={save}/>} {showAi && <AiPlanner tasks={todayTasks} onClose={() => setShowAi(false)}/>}</AppLayout>;
}
