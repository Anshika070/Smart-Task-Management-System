...
export default function App() {
      ...
      <section className="workspace">
        <div className="panel">
          <div className="panel-title">
            <div>
              <h2>{page === "Dashboard" ? "Today's tasks" : "Your tasks"}</h2>
              <p>
          ...
          <button className="add-link" onClick={() => setEditing({})}>
            + Add a task
          </button>
        </div>
        <aside className="side-panel">
          <div className="focus">
            <span>
              <Sparkles size={15} /> AI FOCUS
            </span>
            <h2>Your day, simplified.</h2>
            <p>
              You have <b>{todayTasks.length} tasks</b> planned for today. Start
              with the most important one to build momentum.
            </p>
          ...
          <div className="progress">
            ...
            <div className="progress-graphic">
              <div
                className="circle"
                style={{
                  "--p": `${tasks.length ? (completed / tasks.length) * 360 : 0}deg`,
                }}
              >
                <b>
                  {tasks.length
              ...
              <div>
                ...
                <p>
                  Remaining <b>{tasks.length - completed}</b>
                </p>
              </div>
            </div>
          </div>
  ...
  return (
    ...
    >
      {page === "Categories" ? (
        <CategoryOverview tasks={tasks} onOpenCategory={openCategory} />
      ) : (
        tasksView
      )}
      {editing && (
      ...
      {showAi && (
        <AiPlanner tasks={todayTasks} onClose={() => setShowAi(false)} />
      )}
    </AppLayout>
  );
}
