...
export default function CategoryOverview({ tasks, onOpenCategory }) {
  return <section className="categories-view">
    <div className="view-intro"><h2>Organize by category</h2><p>See where your time and attention are going.</p></div>
    <div className="category-grid">{categoryConfig.map(({ name, icon: Icon, tone }) => {
      ...
      return <article className={`category-card ${tone}`} key={name}>
        <span className="category-icon"><Icon size={22}/></span><h3>{name}</h3>
        <p>{items.length} task{items.length === 1 ? '' : 's'} · {completed} completed</p>
